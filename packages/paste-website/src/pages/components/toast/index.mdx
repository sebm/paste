...
import {Toast, ToastContainer} from '@twilio-paste/toast';
import {DoDont, Do, Dont} from '../../../components/DoDont';
import {Callout, CalloutTitle, CalloutText} from '../../../components/callout';
import {Codeblock} from '../../../components/codeblock';
import {SidebarCategoryRoutes} from '../../../constants';
import {showHideExample} from '../../../component-examples/ToastExamples';
...
<ComponentHeader
  ...
  storybookUrl="https://twilio-labs.github.io/paste/?path=/story/components-toast--neutral"
  abstractUrl="https://share.goabstract.com/00bb0645-23b2-47aa-b57e-b0c6b1da9e8f"
...
For [toasts that automatically time out](#toast-with-automatic-time-out), consider users with different reading speeds, vision levels, and literacy levels (more details in the research insight below).

We recommend giving users about 0.5 seconds per word. For short toasts at 5–10 words, that's about 2–4 seconds until you time-out a toast. For toasts at our [max suggested length of 140 characters](#composition-notes), that's about 30 seconds for about 55 English words. In this case, consider not making the toast automatically time out since the longer content length could also suggest that there's information worth persisting for the customer.

Also consider the time users need to make a decision based on the message. If a toast has any interactive element in it, like a link or button, don't add an automatic time-out to the toast.
...
Provide an "undo" action in the toast if it's easily and safely reversible, and you're able to take the user back to their previous state. For example, allow the user to undo an action through a toast if:

- They submitted a short form that updates an object. Undoing the action can take them back to their filled form with any sensitive fields cleared.

- They deleted non-sensitive information we'd want them to easily retrieve. However if a user accidentally deletes sensitive information, instead of a quick undo action, consider providing other safeguards before they can delete something, such as a modal that asks them to type in a confirmation.
...
<Callout>
  ...
  <CalloutText>
    Add an automatic time-out to a success toast if the only thing it does it confirm that the action was completed. If
    it has any additional information or interactive content, like a link or button, the toast shouldn't time-out.
  </CalloutText>
  <CalloutText>
    Anecdotal data have also suggested that some Twilio customers find it annoying to have to manually close success
    toasts.
  </CalloutText>
</Callout>
...
<LivePreview scope={{Toast, Text, Anchor}} language="jsx">
  {`<Toast onDismiss={() => alert('dismissed')} variant="warning">
  <Text as="span">
    The phone number has been deleted. Please verify there are no functions tied to that number <Anchor href="#">here</Anchor>.
  </Text>
</Toast>`}
</LivePreview>
...
### Toast with automatic time-out

Add an automatic time-out to a toast if it doesn't communicate any information that would help a user continue a flow, and would be annoying to the user to have to explicitly acknowledge and close.

In most cases, it's better to dismiss the toast when a user reloads or navigates away from a page, rather than timing it out. Add an automatic time-out to a success toasts if the only thing it does it confirm that the action was completed. If the toast has any additional information or interactive content, like a link or button, don't make the toast time out.
...
We recommend giving users about 0.5 seconds per word before timing out a toast.

### Showing and Hiding a Toast

For a basic method of hiding and showing, you can use the `hidden` HTML attribute. Passing a true or false value to that attribute will show or hide the Toast respectively.
We'll be providing a more comprehensive method for displaying Toasts in the near future.
...
Keep toast text brief and scannable by placing only the highest priority information in it. Too much text in a toast can overwhelm a user. Aim for no more than 140 characters in a toast, and keep the text directly related to the user's current action. Success toasts generally run about 50 characters long, while all other alerts might run about 100 characters long. However, don't [sacrifice being direct and straightforward](/content/product-style-guide#be-direct-and-straightforward) for a shorter message.

Be specific in your response to the user's action and if applicable, provide a resolution by using an [anchor](/components/anchor/). Be cautious about using a button in a toast since it may compete with other buttons on the screen and should be used only to trigger an action. If you must use a button, use a single [small secondary button](/components/button#small-button) in a toast.

<Callout variant="warning">
  <CalloutTitle>Don't forget the ToastContainer</CalloutTitle>
  <CalloutText>
    In order to control placement for Toasts, Paste provides you with a ToastContainer. The ToastContainer only handles
    the positioning of Toasts in the top right area of the application. Multiple Toasts can be added to a single
    ToastContainer. This ensures the correct spacing is added to each Toast.
  </CalloutText>
</Callout>

## When to use a Toast

Help users avoid scenarios where more than 3 toasts could show at a time. If more than 3 could appear at a time, show only one toast that summarizes the information you were intending to show in multiple toasts.
...
- You need to provide links or buttons contextually to an in-page element.
- You need to communicate dynamically changing content inside a single toast, including showing progress bars. These cases should use a non-modal dialog instead.
...
| border-left-color | <ul><li>Error: color-border-error-light</li><li>Neutral: color-border-neutral</li><li>Success: color-border-success-light</li><li>Warning: color-border-warning-light</li></ul>                       | No          |
